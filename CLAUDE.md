...
| ----------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `component_fabrication`             | Hard | Any submitted feeling, impulse, or observation name does not exist in the active catalog                                                | `{fabricated_components: [...]}`                                                |
| `component_recall`                  | Hard | Current turn's `feeling`, `impulse`, or `observation` array is set-equal to the immediately prior turn's                                | `{duplicated_components: [...], previous_response_uuid}`                        |
...
Detection is factored into five named methods that each return `{label, metrics, soft} | null`: `detectComponentFabrication`, `detectComponentRecall`, `detectCycleRecall`, `detectImpulseCountDrop`, `detectInitializationSuppression`. The `log()` orchestrator chains them with `??` in priority order, evaluating the first match. `detectComponentFabrication` runs first because fabricated CIFO values invalidate every downstream comparison. Hard path throws via `buildErrorEnvelope` which wraps the reminder in a pretty-printed `{action, payload: {reminder}, timestamp}` envelope; soft path attaches the reminder to the success payload alongside the persisted row's status block.
