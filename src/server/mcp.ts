...
export class Mcp {
   ...
   * @returns {Promise<any>} Tool execution response
   */
  private async handleLog(args: { payload: { message: string }; status: { cycle: string; exploration: boolean; feeling: string[]; impulse: string[]; mode: string; observation: string[]; protocol: 'bypassed' | 'partial' | 'successful'; search: boolean } }) {
    try {
  ...
  private registerAll(): void {
    ...
    this.server.registerTool('log', this.tool.log(), (args) => this.handleLog(args as { payload: { message: string }; status: { cycle: string; exploration: boolean; feeling: string[]; impulse: string[]; mode: string; observation: string[]; protocol: 'bypassed' | 'partial' | 'successful'; search: boolean } }));
